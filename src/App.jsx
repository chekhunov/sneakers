...
const App = () => {
    ...
    React.useEffect(() => {
        ...
        //         setItems(json)
        //     })
        axios.get('https://60de45f9878c890017fa2e50.mockapi.io/items')
             .then(res => {
                 setItems(res.data)
             })
        //запрос на корзину
        axios.get('https://60de45f9878c890017fa2e50.mockapi.io/cart')
             .then(res => {
                 setCartItems(res.data)
             })
        axios.get('https://60de45f9878c890017fa2e50.mockapi.io/favorites')
             .then(res => {
...
//async ассинхронная функция для того чтоб дождаться ответа от сервера и выполнить запрос пост
    const onAddToFavorite = async (obj) => {
        try {
            if (favorites.find(favObj => favObj.id === obj.id)) {
                ...
                const {data} = await
                    axios.post('https://60de45f9878c890017fa2e50.mockapi.io/favorites', obj);
    ...
    return (
        <div className="wrapper clear" >
            ...
            {/*<Count />*/}
            <Route path={'/'} exact ><Home items={items}
            ...
            <Route path={'/favorites'} exact ><Favorites items={favorites}
                                                         onAddToFavorite={onAddToFavorite}
            /></Route >
        </div >
    );
